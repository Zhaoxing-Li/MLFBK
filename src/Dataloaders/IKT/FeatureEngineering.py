...
def k_means_clust(session, train_students, max_stu, max_seg, num_clust, num_skills, num_iter):
    identifiers = 3
    max_stu = int(max_stu)
    max_seg = int(max_seg)
...
def read_data_from_csv_file(trainfile):
    rows = []
    max_skills = 0
    max_steps = 0 
    max_items = 0
    ...
    print("starting BKT training layer")
    ...
    return train_students, studentids, max_skills, max_items, train_ids
...
def get_features(students, item_diff, max_stu, cluster, num_skills, datatype):
    ...
    data = pd.DataFrame({'user_id': stu_list, 'item_id': item_list ,'skill_id': p0_list,'skill_mastery': p1_list, 'ability_profile': p2_list, 'problem_difficulty': p3_list, 'correctness': p4_list})
    return data

def main(file_path):
    cluster_num = 7
    
    print("starting feature engineering layer on", file_path)

    train_students, student_ids, max_skills, max_items, train_ids = read_data_from_csv_file(file_path)
                 
    item_diff = difficulty_data(train_students, max_items)           

    train_cluster_data, train_max_seg = cluster_data(train_students, max(train_ids) + 1,max_skills,"train")        
    
    max_stu = max(student_ids)+1
    max_seg = int(train_max_seg) + 1
    
    train_result_data = None
    test_result_data = None
    with tf.Session() as session:        
        cluster = k_means_clust(session, train_cluster_data, max_stu, max_seg, cluster_num, max_skills, 40)
        train_result_data = get_features(train_students, item_diff, max_stu, cluster, max_skills, "train" )

    return_data = train_result_data.append(test_result_data, ignore_index=True)
    return return_data
