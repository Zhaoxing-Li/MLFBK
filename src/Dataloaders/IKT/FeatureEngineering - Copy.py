...
def read_data_from_csv_file(trainfile, testfile):
    ...
    # cut down rows for more efficiency REMOVE THIS EVENTUALLY
    rows = rows[:100]
    ...
    print("finished BKT training layer")
    index = 0   
    tuple_rows = []
    while(index < len(rows)):
          if int(rows[index][0])>problem_len: 
...
def main(unused_thing):
    cluster_num= 7
    problem_len= 20
    
    data_folder = '../../../data/EdNetKT1/'
    train_data= data_folder+data_name+'_train.csv'
    test_data= data_folder+data_name+'_test.csv'
    ...
    with tf.Session() as session:
        ...
        test_result_data = get_features(test_students, item_diff, max_stu, cluster, max_skills, "test")
