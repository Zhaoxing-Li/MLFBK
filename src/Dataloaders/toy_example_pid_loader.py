...
DATASET_DIR = "../data/toy_example/preprocessed_df.csv"
IKT_DIR = "../data/toy_example/IKT/"
...
class IKT_HANDLER:
    ...
    def format_to_ikt(self):
        ...
        for user in users:
            ...
            file_name = IKT_DIR + f'toy_ikt_{self.save_name}.csv'
...
def TOY_PID_SPLIT(max_seq_len, dataset_dir=DATASET_DIR):
    ...
    # create the TOY loaders from this data
    validation_loader = TOY_PID(max_seq_len, valid_data, "valid", fe)
    train_loader = TOY_PID(max_seq_len, train_data, "train", fe)
    test_loader = TOY_PID(max_seq_len, test_data, "test", fe)

    return train_loader, test_loader, validation_loader, fe

if __name__ == "__main__":
    max_s_len = 100
    pd.read_csv(DATASET_DIR)
    loader = TOY_PID(max_s_len)
