...
def pid_collate_fn(batch, pad_val=-1):
    ...
    for q_seq, r_seq, pid_seq, ap_seq, pd_seq, sm_seq in batch:

        q_seqs.append(torch.Tensor(q_seq)) 
        r_seqs.append(torch.Tensor(r_seq)) 
        pid_seqs.append(torch.Tensor(pid_seq)) 
        ap_seqs.append(torch.Tensor(ap_seq))
        pd_seqs.append(torch.Tensor(pd_seq))
        sm_seqs.append(torch.Tensor(sm_seq))
    ...
    ap_seqs = pad_sequence(
        ap_seqs, batch_first=True, padding_value=pad_val
    )
    pd_seqs = pad_sequence(
        pd_seqs, batch_first=True, padding_value=pad_val
    )
    sm_seqs = pad_sequence(
        sm_seqs, batch_first=True, padding_value=pad_val
    )

    mask_seqs = (q_seqs != pad_val)

    q_seqs, r_seqs, pid_seqs, ap_seqs, pd_seqs, sm_seqs = q_seqs * mask_seqs, r_seqs * mask_seqs, pid_seqs * mask_seqs, ap_seqs * mask_seqs, pd_seqs * mask_seqs, sm_seqs * mask_seqs

    return q_seqs, r_seqs, pid_seqs, ap_seqs, pd_seqs, sm_seqs, mask_seqs
...
# early stop
class EarlyStopping:
    def __init__(self, metric_name, best_score=0, patience=20, verbose=True, delta=0, path='../checkpoints/checkpoint.pt'):
