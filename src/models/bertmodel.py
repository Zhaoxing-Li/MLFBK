...
class BERT(nn.Module):
    ...
    def __init__(
        ...
        self.emb_ap = nn.Embedding(self.num_ap, self.hidden_size).to(self.device)
        # problem difficulty embedding
        self.emb_pd = nn.Embedding(self.num_pd, self.hidden_size).to(self.device)
        # skill mastery embedding
        self.emb_sm = nn.Embedding(self.num_sm, self.hidden_size).to(self.device)
    ...
    def forward(self, q, r, pid, ap, pdf, sm, mask):
        # |q| = (bs, n)
        # |r| = (bs, n)
        # |mask| = (bs, n)
        # pid -> question
        ...
        # r -> response
        # ap -> ability profile
        # pdf -> problem difficulty
        # sm -> skill mastery
        # print(sm[0])

        emb = self.emb_q(q) + self.emb_r(r) + self.emb_pid(pid) + self.emb_ap(ap) \
            + self.emb_pd(pdf) + self._positional_embedding(q) + self.emb_sm(sm)
        # |emb| = (bs, n, emb_size)
